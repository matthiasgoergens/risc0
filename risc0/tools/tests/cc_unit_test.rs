...
fn reset_env() {
    // Some tests check that a flag is *not* present.  These tests might fail if the
    // flag is set in the CFLAGS or CXXFLAGS environment variables.  This
    // function clears the CFLAGS and CXXFLAGS variables to make sure that the
