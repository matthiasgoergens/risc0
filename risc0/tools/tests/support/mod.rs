...
///
/// We first try to hard link an executable to save space. If that fails (as on
/// Windows with different mount points, issue #60), we copy.
...
/// Copy an executable.
///
/// On macOS, hard linking the executable leads to strange failures (issue
/// #419), so we just copy.
#[cfg(target_os = "macos")]
fn link_or_copy<P: AsRef<Path>, Q: AsRef<Path>>(from: P, to: Q) -> io::Result<()> {
